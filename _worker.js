...
async function 生成本地订阅(host, UUID, noTLS, newAddressesapi, newAddressescsv, newAddressesnotlsapi, newAddressesnotlscsv) {
    ...
    if (noTLS == 'true') {
        ...
        const notlsPromises = uniqueAddressesnotls.map(async addressEntry => {
            ...
            const createConfig = (ip, p, id) => {
                if (!isValidIPv4(ip) && p == "-1") {
                ...
                const ctx = `${协议类型}://${UUID}@${ip}:${p + atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT0mdHlwZT13cyZob3N0PQ==') + 伪装域名}&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(id + 节点备注)}`;
                return ctx;
            };

            if (!isValidIPv4(address)) {
                const ips = await fetchIPv4s(address);
                if (ips.length > 0) {
                    return ips.map((ip, index) => createConfig(ip, port, `${addressid}-${index + 1}`));
                }
                return null; // Drop unresolved domains
            }
            return createConfig(address, port, addressid);
        });
        const notlsArrays = await Promise.all(notlsPromises);
        notlsresponseBody = notlsArrays.flat().filter(Boolean).join('\n');
    }

    // 使用Set对象去重
    const uniqueAddresses = [...new Set(addresses)];

    const responsePromises = uniqueAddresses.map(async addressEntry => {
        let address = addressEntry;
        let port = "-1";
        let addressid = address;
        ...
        const createConfig = (ip, p, id) => {
            ...
            */
            const 协议类型 = atob(啥啥啥_写的这是啥啊);
            const ctx = `${协议类型}://${UUID}@${ip}:${p + atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT10bHMmc25pPQ==') + 伪装域名}&fp=random&type=ws&host=${伪装域名}&path=${encodeURIComponent(最终路径) + allowInsecure}&fragment=${encodeURIComponent('1,40-60,30-50,tlshello')}#${encodeURIComponent(id + 节点备注)}`;
    ...
    const responseBody = responseArrays.flat().filter(Boolean).join('\n');
...
async function fetchIPv4s(domain) {
    try {
        ...
        if (!response.ok) {
            console.log(`DNS查询失败 for ${domain}`);
            return [];
        }
        ...
        if (ipv4s.length === 0) {
            console.log(`未找到IPv4地址 for ${domain}`);
        }
        return ipv4s;
    } catch (e) {
        console.error(`fetchIPv4s error for ${domain}:`, e);
        return [];
    }
}
